...
var get_countries = require('../views/sprint89/scripts/countries.js');
...
router.post('/secure', function(req, res) {
  if ( req.body['address-select'] === 'Yes' ) {
    res.redirect('/sprint89/pension-age');
  } else {
    res.redirect('/sprint89/currentaddress');
  }
});
...
router.get('/have-you-lived-abroad', function(req, res) {
  res.render('sprint89/have-you-lived-abroad');
});
...
router.get('/what-countries-have-you-lived-in', function(req, res) {
  res.render('sprint89/what-countries-have-you-lived-in');
});
...
router.get('/tell-us-about-lived', function(req, res) {
  ...
  if ( countries.length === 0 ) {
    ...
    if ( countryType.resident || countryType.insurance ) {
      res.render('sprint89/tell-us-about-lived', {country: country, countryType: countryType, step: step});
    } else {
      res.cookie('c-lived-count', countries.length);
      res.cookie('c-lived-list', countries);
...
router.get('/have-you-worked-abroad', function(req, res) {
  ...
  res.render('sprint89/have-you-worked-abroad', {title: title});
});
...
router.get('/what-countries-have-you-worked-in', function(req, res) {
  res.render('sprint89/what-countries-have-you-worked-in');
});
...
router.get('/tell-us-about-worked', function(req, res) {
  ...
  if ( countries.length === 0 ) {
    ...
    if ( countryType.insurance ) {
      res.render('sprint89/tell-us-about-worked', {country: country, step: step});
    } else {
      res.cookie('c-worked-count', countries.length);
      res.cookie('c-worked-list', countries);
...
router.get('/relationship-status', function(req, res) {
    res.render('sprint89/relationship-status');
})
...
router.all('/relationship-status-date/:type', function(req, res) {
  if (req.body.submit === "Continue") {
    res.redirect("/sprint89/relationship-status-more/"+req.params.type);
  }else{
    var isMarried = false,
        isEnded = false;
    ...
    res.render('sprint89/relationship-status-date', {
      eventText: eventText,
...
router.all('/relationship-status-more/:type', function(req, res) {
  if (req.body.submit === "Continue") {
    res.redirect("/sprint89/benefits");
  }else{
    var isMarried = false,
        isEnded = false,
        widowed = false;
    ...
    res.render('sprint89/relationship-status-more',{type: req.params.type,pageHeader: pageHeader,isEnded: isEnded, isMarried: isMarried, widowed: widowed });
  }
});

// Contact page
router.get('/contact', function(req, res) {
  res.render('sprint89/contact');
});
...
router.get('/unhappy-ending', function(req, res) {
  ...
  res.render('sprint89/unhappy-ending');
});

router.get('/end', function(req, res) {
  var completeDate = get_todays_date();

  res.render('sprint89/end', {completeDate: completeDate});
});
