...
var get_countries = require('../views/northernireland/scripts/countries.js');

var forceFail = false;

// Secure page with invite code
router.get('/secure', function(req, res) {
  res.render('northernireland/secure');
});
...
router.post('/secure', function(req, res) {
  ...
  if (req.body.reference.toLowerCase() === 'EDMO435HN3'.toLowerCase()) {
    res.redirect('/northernireland/pension-age');
  } else {
    res.redirect('/northernireland/pension-age');
  }
});

router.post('northernireland/secure', function(req, res) {
  res.locals.error = true;
...
router.get('/have-you-lived-abroad', function(req, res) {
  res.render('northernireland/have-you-lived-abroad');
});
...
router.get('/what-countries-have-you-lived-in', function(req, res) {
  res.render('northernireland/what-countries-have-you-lived-in');
});
...
router.get('/tell-us-about-lived', function(req, res) {
  ...
  if ( countries.length === 0 ) {
    ...
    if ( countryType.resident || countryType.insurance ) {
      res.render('northernireland/tell-us-about-lived', {country: country, countryType: countryType, step: step});
    } else {
      res.cookie('c-lived-count', countries.length);
      res.cookie('c-lived-list', countries);
...
router.get('/have-you-worked-abroad', function(req, res) {
  ...
  res.render('northernireland/have-you-worked-abroad', {title: title});
});
...
router.get('/what-countries-have-you-worked-in', function(req, res) {
  res.render('northernireland/what-countries-have-you-worked-in');
});
...
router.get('/tell-us-about-worked', function(req, res) {
  ...
  if ( countries.length === 0 ) {
    ...
    if ( countryType.insurance ) {
      res.render('northernireland/tell-us-about-worked', {country: country, step: step});
    } else {
      res.cookie('c-worked-count', countries.length);
      res.cookie('c-worked-list', countries);
...
router.all('/relationship-status', function(req, res) {
  if (req.body.relationship === "Never been married") {
    ...
    res.render('northernireland/relationship-status');
  }
});
...
router.get('/relationship-status-date/:type', function(req, res) {
  ...
  res.render('northernireland/relationship-status-date', {
    eventText: eventText,
...
router.all('/relationship-status-more/:type', function(req, res) {
  if (req.body.submit === "Continue") {
    ...
    res.render('northernireland/relationship-status-more',{type: req.params.type,pageHeader: pageHeader,isEnded: isEnded, isMarried: isMarried, widowed: widowed });
  }
});

// Contact page
router.get('/contact', function(req, res) {
  res.render('northernireland/contact');
});
...
router.get('/bank-details', function(req, res) {
  res.render('northernireland/bank-details');
});
...
router.post('/bank-details', function(req, res) {
  ...
  if ( blank_fields === 0 && forceFail === false) {
    ...
    if ( fail_attempts >= 2 ) {
      ...
      res.render('northernireland/bank-details', {errors: errors});
    }
  }
});
...
router.get('/unhappy-ending', function(req, res) {
  req.session.view = null;
  res.render('northernireland/unhappy-ending');
});
...
router.get('/settings', function(req, res) {
  res.render('northernireland/settings');
});
