...
var get_countries = require('../views/sprint9v4/scripts/countries.js');

var forceFail = false;

// Secure page with invite code
router.get('/secure', function(req, res) {
  res.render('sprint9v4/secure');
});
...
router.post('/secure', function(req, res) {
  ...
  if (req.body.reference.toLowerCase() === 'EDMO435HN3' .toLowerCase()) {
    res.redirect('/sprint9v4/dob-check');
  } else {
    res.redirect('/sprint9v4/pension-age');
  }
});
...
router.post('/dob-check', function(req, res) {
  if (req.body["dob-year"] === "1952") {
    return res.redirect("/sprint9v4/pension-age-nv")
  }

  if (req.body["dob-year"] === "1953") {
    return res.redirect("/sprint9v2/dob-early")
  }

  if (req.body["dob-year"] === "1950") {
    return res.redirect("/sprint9v2/dob-rules")
  }
...
router.get('/have-you-lived-abroad', function(req, res) {
  res.render('sprint9v4/have-you-lived-abroad');
});
...
router.get('/what-countries-have-you-lived-in', function(req, res) {
  res.render('sprint9v4/what-countries-have-you-lived-in');
});
...
router.get('/tell-us-about-lived', function(req, res) {
  ...
  if ( countries.length === 0 ) {
    ...
    if ( countryType.resident || countryType.insurance ) {
      res.render('sprint9v4/tell-us-about-lived', {country: country, countryType: countryType, step: step});
    } else {
      res.cookie('c-lived-count', countries.length);
      res.cookie('c-lived-list', countries);
...
router.get('/have-you-worked-abroad', function(req, res) {
  ...
  res.render('sprint9v4/have-you-worked-abroad', {title: title});
});
...
router.get('/what-countries-have-you-worked-in', function(req, res) {
  res.render('sprint9v4/what-countries-have-you-worked-in');
});
...
router.get('/tell-us-about-worked', function(req, res) {
  ...
  if ( countries.length === 0 ) {
    ...
    if ( countryType.insurance ) {
      res.render('sprint9v4/tell-us-about-worked', {country: country, step: step});
    } else {
      res.cookie('c-worked-count', countries.length);
      res.cookie('c-worked-list', countries);
...
router.all('/relationship-status', function(req, res) {
  if (req.body.relationship === "Never been married") {
    ...
    res.render('sprint9v4/relationship-status');
  }
});
...
router.all('/relationship-status-date/:type', function(req, res) {
  if (req.body.submit === "Continue") {
    res.redirect("/sprint9v4/relationship-status-more/"+req.params.type);
  }else{
    var isMarried = false,
        isEnded = false;
    ...
    res.render('sprint9v4/relationship-status-date', {
      eventText: eventText,
...
router.all('/relationship-status-more/:type', function(req, res) {
  if (req.body.submit === "Continue") {
    res.redirect("/sprint9v4/contact");
  }else{
    var isMarried = false,
        isEnded = false,
        widowed = false;
    ...
    res.render('sprint9v4/relationship-status-more',{type: req.params.type,pageHeader: pageHeader,isEnded: isEnded, isMarried: isMarried, widowed: widowed });
  }
});

// Contact page
router.get('/contact', function(req, res) {
  res.render('sprint9v4/contact');
});
...
router.get('/bank-details', function(req, res) {
  res.render('sprint9v4/bank-details');
});
...
router.post('/bank-details', function(req, res) {
  ...
  if ( blank_fields === 0 && forceFail === false) {
    ...
    if ( fail_attempts >= 2 ) {
      ...
      res.render('sprint9v4/bank-details', {errors: errors});
    }
  }
});
...
router.get('/unhappy-ending', function(req, res) {
  req.session.view = null;
  res.render('sprint9v4/unhappy-ending');
});
...
console.log(req.session)
  if (req.session.data["dob-year"] === "1952") {
    return res.redirect("/sprint9v4/end-nonverified")
  }

  res.render('sprint9v4/end', {completeDate: completeDate});
});
...
console.log(req.session)
  ...
  res.render('sprint9v4/end-nonverified', {completeDate: completeDate});
});
